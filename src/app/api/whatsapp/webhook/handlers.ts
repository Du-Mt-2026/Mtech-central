/**
 * Webhook Event Handlers — Evolution Go v3
 *
 * Extracted from src/app/api/whatsapp/webhook/route.ts (P2.1 refactor).
 *
 * This module contains all event handlers for Evolution Go webhook events.
 * Each `case` in the dispatch switch corresponds to a specific event type
 * sent by Evolution Go (Connected, Disconnected, Message, SEND_MESSAGE_ACK,
 * Receipt, QRCode, INSTANCE_DELETED, etc.).
 *
 * Public API:
 *   dispatchWebhookEvent(ctx) — handles one webhook event, returns void
 *
 * Context shape:
 *   - event: string (event name from Evolution Go)
 *   - data: any (event payload)
 *   - instanceId: string (UUID of the instance)
 *   - chipInstanceName: string (resolved instance name)
 *   - linkedChip: Chip | null (chip linked to this instance, if any)
 *   - banSettings: { banCodes, restrictionKeywords, banLookbackMs, banMaxMessagesCheck, banKeywordThreshold }
 *
 * Side effects:
 *   - Reads/writes DB (chip status, inbox messages, conversations, group metadata, audit logs)
 *   - Calls Evolution API (fetchProfilePicture, enableRejectCallAfterConnection, fetchInstances)
 *   - Calls reconnection queue (enqueueReconnection, markChipReconnected, dequeueReconnection)
 *   - Updates QR cache (setQRCode, clearQRCode)
 *   - Broadcasts to SSE clients (broadcastToChip)
 *   - Removes WireGuard peers (removeWireGuardPeer)
 *
 * All handlers are async and may throw — caller is responsible for catching
 * and returning 200 to Evolution Go (to avoid retry storms).
 */
...
interface WebhookContext {
  ...
  linkedChip: any | null  // Prisma Chip type
 ...
 *
 * This is the ONLY public function. The route.ts calls this after
 * authenticating the request and resolving the chip context.
 */
export async function dispatchWebhookEvent(ctx: WebhookContext): Promise<void> {
  const { event, data, instanceId, chipInstanceName, linkedChip, banSettings } = ctx
  const { banCodes, restrictionKeywords, banLookbackMs, banMaxMessagesCheck, banKeywordThreshold } = banSettings
